from scapy.all import Ether, ARP, IP, UDP, srp1, Raw, Padding
import os
import random
import time
...
def run_stress_test():
    ...
    # Append 18 bytes of padding to reach the 60-byte Ethernet minimum
    base_arp = Ether(dst="ff:ff:ff:ff:ff:ff") / ARP(pdst=FPGA_IP, psrc=PC_IP)

    # 2. Calculate exactly how many bytes are needed to reach 60 bytes
    pad_length = 60 - len(base_arp)

    # 3. Append the zeros as a RAW payload so Linux cannot strip it
    ...
    for i in range(NUM_PACKETS):
        ...
        packet = (Ether(dst="00:1A:2B:3C:4D:5E")/ 
